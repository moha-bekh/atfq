...
async fn test_oauth_registration_and_login_it() {
    ...
    // Clean up
    sqlx::query("DELETE FROM users WHERE email = $1").bind(email).execute(&pool).await.unwrap();
    sqlx::query("DELETE FROM user_oauth WHERE provider = $1 AND provider_id = $2").bind(provider_name).bind(provider_id).execute(&pool).await.unwrap();
    ...
    // Verify in DB
    let user_in_db = repo.find_by_oauth_id(provider_name, provider_id).await.unwrap().expect("User should be found in DB");
    assert_eq!(user_in_db.id, res.user.id);

    // 3. Callback (Existing User)
    cache.set("oauth_state:test_state2", "pending", std::time::Duration::from_secs(60)).await.unwrap();
    let res2 = uc.handle_callback(provider_name, provider, "code".into(), "test_state2".into()).await.unwrap();
